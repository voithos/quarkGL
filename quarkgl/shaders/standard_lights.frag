...
#endif
uniform QrkDirectionalLight directionalLights[QRK_MAX_DIRECTIONAL_LIGHTS];

#ifndef QRK_MAX_POINT_LIGHTS
#define QRK_MAX_POINT_LIGHTS 1
#endif
uniform QrkPointLight pointLights[QRK_MAX_POINT_LIGHTS];

#ifndef QRK_MAX_SPOT_LIGHTS
#define QRK_MAX_SPOT_LIGHTS 1
#endif
uniform QrkSpotLight spotLights[QRK_MAX_SPOT_LIGHTS];
...
vec3 qrk_shadeAllDirectionalLights(QrkMaterial material, vec3 fragPos,
  ...
  for (int i = 0; i < QRK_MAX_DIRECTIONAL_LIGHTS; i++) {
    result += qrk_shadeDirectionalLight(material, directionalLights[i], fragPos,
                                        normal, texCoords);
  }
  return result;
}
...
vec3 qrk_shadeAllPointLights(QrkMaterial material, vec3 fragPos, vec3 normal,
  ...
  for (int i = 0; i < QRK_MAX_POINT_LIGHTS; i++) {
    result += qrk_shadePointLight(material, pointLights[i], fragPos, normal,
                                  texCoords);
  }
  return result;
}
...
vec3 qrk_shadeAllSpotLights(QrkMaterial material, vec3 fragPos, vec3 normal,
  ...
  for (int i = 0; i < QRK_MAX_SPOT_LIGHTS; i++) {
    result +=
        qrk_shadeSpotLight(material, spotLights[i], fragPos, normal, texCoords);
  }
  return result;
}
